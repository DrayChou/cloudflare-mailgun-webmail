...
function appPage(title: string, user: User, content: string, status = 200): Response {
  const adminLink = user.is_admin ? `<a href="/admin/users">♙<span>邮箱账号</span></a>` : "";
  return htmlPage(title, `<div class="app-shell"><aside class="sidebar"><a class="logo" href="/inbox"><span>M</span><strong>Mini Mail</strong></a><nav><a href="/inbox">⌂<span>收件箱</span></a><a href="/sent">↗<span>已发送</span></a><a href="/compose">✎<span>写邮件</span></a><form method="post" action="/sync"><button class="nav-button">↻<span>立即同步</span></button></form><a href="/settings/password">⚙<span>账户安全</span></a>${adminLink}</nav><div class="account"><span class="avatar">${escapeHtml(user.username.charAt(0).toUpperCase())}</span><div><strong>${escapeHtml(user.username)}</strong><small>${user.is_admin ? "管理员" : "邮箱用户"}</small></div><form method="post" action="/logout"><button title="退出登录">↪</button></form></div></aside><main class="main-content">${content}</main></div>`, status);
}
